...
def _display_analysis_results(analysis: Dict[str, Any]):
    ...
    if response:
        ...
        # Add audio player for AI response if available
        response_audio_path = analysis.get("responseAudioPath")
        if response_audio_path and os.path.exists(response_audio_path):
            st.subheader("🔊 AI Response Audio")
            try:
                audio_file = open(response_audio_path, "rb")
                st.audio(audio_file.read(), format="audio/mp3")
                st.success("✅ AI response audio generated successfully!")
            except Exception as e:
                st.warning(f"⚠️ Could not load audio file: {e}")
        elif response_audio_path:
            st.warning("⚠️ Audio file not found. Voice agent may not be available.")
...
def main():
    ...
    - **🔊 AI Response Audio** - Text-to-speech audio of the AI response (when voice agent is available)
    ...
    # Footer
    st.markdown("---")
    st.caption("🎯 Pitch Analysis Agent - Powered by **Mistral AI** for comprehensive analysis | ElevenLabs for speech-to-text & text-to-speech")
